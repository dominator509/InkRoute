...
- Web/dashboard: Vercel projects connected to the monorepo (sandbox; production gated on evidence).
...
Sentry / fallback reporter / OpenTelemetry
  ...
  -> dashboard triage
  -> alert provider
  -> approved GitHub issue / agent handoff (review-gated)
```
...
7. Privacy request workers implement verified export/delete/rectification (evidence-gated) with retention/legal-hold rules.
