...
| Production | Live SaaS | Vercel + managed Postgres + storage + Stripe live (gated) |
...
- Sentry/alerts configured (gated; production requires evidence).
- Backups verified.
...
1. Stripe test-mode account configured (sandbox) with `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, and dashboard webhook endpoint.
...
1. Sentry projects exist for web/dashboard/mobile or a deliberate shared-project strategy is approved (gated).
2. `SENTRY_AUTH_TOKEN`, org/project slugs, DSNs, webhook secrets, and alert webhooks are configured in deployment (gated)ent secrets.
...
- No Vercel, EAS, Sentry, Search Console, or database deployment secret is configured (none; gated).
