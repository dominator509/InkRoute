...
| `POST` | `api/appointments` | Create appointment from booking | `booking:write` | `appointmentInputSchema` | DB-backed route added: validates booking/artist/client/travel/studio tenant scope, claims/replays IdempotencyKey results, persists Appointment + BookingStateEvent + AuditLog + local NotificationJob handoff intent in one transaction, advances scheduling state through the booking transition plan, and returns deferred deposit/calendar/provider-notification lifecycle intents. Production local fallback fails closed; provider execution and tenant-isolated integration tests remain evidence-gated. |
