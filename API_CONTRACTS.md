...
| `GET` | `api/public/:tenantSlug/reviews` | Approved public testimonials (moderation-approved; legal review pending) | `publicReadQuerySchema` bounded `limit` | No | DB-oriented tenant-scoped approved-review read with validated bounded query limit and non-production demo fallback; production disables local fallback. |
