...
export async function POST(request: Request, context: { params: Promise<{ tenantSlug: string }> }) {
  ...
  if (tenantSlug !== inkrouteDemoTenant.slug) {
    return NextResponse.json({ ok: false, error: { code: "TENANT_NOT_FOUND", message: "Tenant analytics scope was not found." } }, { status: 404 });
  }

  const body = (await request.json().catch(() => ({}))) as Record<string, unknown>;
  const name = stringValue(body.name) as AnalyticsEventName | undefined;
  if (!name || !allowedEvents.has(name)) {
    return NextResponse.json({ ok: false, error: { code: "INVALID_EVENT", message: "Unsupported SEO analytics event." } }, { status: 400 });
  }
