...
const sensitiveValidatorLaunchAdoptionKeyPattern =
  /(token|secret|password|authorization|cookie|email|phone|name|address|medical|payment|card|consent|tenant|user|client|provider|webhook|database|url|uri|dsn|key|id|payload|artifact|metadata|repository|repo|branch|pull|pr|reviewer|codeowner)/iu;
