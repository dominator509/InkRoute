...
describe("dashboard security runtime middleware wiring", () => {
  it("applies shared security header and CSRF enforcement plans at the dashboard middleware boundary", () => {
    const source = readWorkspaceFile("apps/dashboard/middleware.ts");

    expect(source).toContain('import { buildSecurityRuntimeEnforcementPlan } from "@inkroute/security"');
    expect(source).toContain('appSurface: "dashboard"');
