...
export async function GET(request: NextRequest) {
  ...
  if (tenantId !== actor.tenantId) {
    return NextResponse.json({ ok: false, error: { code: "TENANT_MISMATCH", message: "Cannot query clients for another tenant." } }, { status: 403 });
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: { code: "CLIENT_LIST_READ_FAILED", message: "Clients could not be loaded." } }, { status: 500 });
  }
}
