...
describe("dashboard booking state mutation route contract", () => {
  ...
  it("documents the live API seam from booking list and detail pages", () => {
    ...
    expect(detailSource).toContain("POST /api/bookings/${booking.id}/state");
    expect(detailSource).toContain("BookingStateEvent writes");
    expect(detailSource).toContain("AuditLog persistence");
  });
});
